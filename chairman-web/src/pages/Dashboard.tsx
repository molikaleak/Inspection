...
export default function Dashboard() {
  ...
  const handleSelectInspector = (inspectorName: string) => {
     const idx = activeReports.findIndex(r => r.inspector === inspectorName);
     if (idx !== -1) {
        setCurrentIndex(idx);
        setExpandedSections([0]);
        setSelectedForeman(getForemenForInspector(inspectorName)[0] || mockForemen[0]);
     }
  };
  ...
  const handleNext = () => {
    if (currentIndex < activeReports.length - 1) {
       ...
       setSelectedForeman(getForemenForInspector(activeReports[nextIndex].inspector)[0] || mockForemen[0]);
  ...
  const handlePrev = () => {
    if (currentIndex > 0) {
       ...
       setExpandedSections([0]);
       setSelectedForeman(getForemenForInspector(activeReports[prevIndex].inspector)[0] || mockForemen[0]);
       window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  };
  ...
  const handleFinishReview = () => {
      ...
      if (newUnreviewed.length > 0) {
          setSelectedForeman(getForemenForInspector(newUnreviewed[0].inspector)[0] || mockForemen[0]);
      } else {
    ...
    <div className="flex-1 lg:pl-60 min-h-screen bg-surface transition-all overflow-hidden flex flex-col relative w-full">
      <main className="max-w-3xl mx-auto w-full p-5 md:p-8 space-y-8 pb-32 lg:pb-8">
        ...
        <div className="flex justify-between items-center">
           ...
           <div className="flex gap-3 sm:gap-4 items-center w-full sm:w-auto justify-between sm:justify-end">
              {/* View Mode Switch */}
              <div className="flex gap-1 bg-surface-lowest p-1 rounded-xl border border-on-surface/[0.04]">
                 <button 
                   onClick={() => setViewMode('professional')}
                   className={`px-3 py-1.5 rounded-lg text-[13px] font-medium transition-all ${viewMode === 'professional' ? 'bg-surface shadow-[0_2px_8px_rgba(0,0,0,0.04)] text-on-surface' : 'text-on-surface-variant hover:text-on-surface'}`}
                 >
                   Professional
                 </button>
                 <button 
                   onClick={() => setViewMode('excel')}
                   className={`px-3 py-1.5 rounded-lg text-[13px] font-medium transition-all ${viewMode === 'excel' ? 'bg-surface shadow-[0_2px_8px_rgba(0,0,0,0.04)] text-on-surface' : 'text-on-surface-variant hover:text-on-surface'}`}
                 >
                   Excel Form
                 </button>
              </div>
        ...
        {viewMode === 'excel' ? (
           <div className="space-y-4 animate-in fade-in duration-300">
              <div className="flex flex-col gap-2 mb-6">
                  {/* Row 1: Inspectors */}
                  <div className="flex items-start gap-3 overflow-x-auto no-scrollbar pt-2 -mx-5 px-5 md:mx-0 md:px-0">
                     <span className="text-[10px] uppercase tracking-wider font-bold text-black/30 mt-4 mr-2 shrink-0 w-[60px]">Inspector</span>
                     {availableInspectors.map(name => {
                        const isSelected = report.inspector === name;
                        return (
                        <div 
                          key={name} 
                          onClick={() => handleSelectInspector(name)}
                          className="flex flex-col items-center gap-1.5 cursor-pointer transition-all shrink-0 w-14 group"
                        >
                           <div className={`w-11 h-11 rounded-full ring-2 overflow-hidden transition-all ${isSelected ? 'ring-primary ring-offset-2 scale-110' : 'ring-black/10 opacity-60 group-hover:opacity-100 group-hover:ring-black/30'}`}>
                              <img src={mockInspectorsData[name]?.avatar || mockInspectorsData['Mr. Menghour'].avatar} className="w-full h-full object-cover" alt="Inspector" />
                           </div>
                           <h2 className={`text-[10.5px] leading-tight text-center truncate w-full transition-colors ${isSelected ? 'text-primary font-bold' : 'text-black/50 group-hover:text-black/80'}`}>{name}</h2>
                        </div>
                     )})}
                  </div>
                  
                  {/* Row 2: Foremen */}
                  <div className="flex items-start gap-3 overflow-x-auto no-scrollbar pb-3 pt-3 -mx-5 px-5 md:mx-0 md:px-0 border-t border-black/[0.04]">
                     <span className="text-[10px] uppercase tracking-wider font-bold text-black/30 mt-4 mr-2 shrink-0 w-[60px]">Foreman</span>
                     {getForemenForInspector(report.inspector).map(f => (
                        <div 
                          key={f.id} 
                          onClick={() => setSelectedForeman(f)}
                          className="flex flex-col items-center gap-1.5 cursor-pointer transition-all shrink-0 w-14 group"
                        >
                           <div className={`w-11 h-11 rounded-full ring-2 overflow-hidden transition-all ${selectedForeman.id === f.id ? 'ring-black ring-offset-2 scale-110' : 'ring-black/10 opacity-60 group-hover:opacity-100 group-hover:ring-black/30'}`}>
                              <img src={f.avatar} className="w-full h-full object-cover" alt="Foreman" />
                           </div>
                           <h2 className={`text-[10.5px] leading-tight text-center truncate w-full transition-colors ${selectedForeman.id === f.id ? 'text-black font-bold' : 'text-black/50 group-hover:text-black/80'}`}>{f.name}</h2>
                        </div>
                     ))}
                  </div>
              </div>
              <ExcelFormView report={report} foreman={selectedForeman} checklist={mockChecklist} inspectorImage={mockInspectorsData[report.inspector]?.avatar || mockInspectorsData['Mr. Menghour'].avatar} />
           </div>
        ) : (
           <>
...
function ExcelFormView({ report, foreman, checklist, inspectorImage }: { report: any, foreman: any, checklist: any, inspectorImage?: string }) {
